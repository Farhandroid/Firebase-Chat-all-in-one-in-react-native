...
export default class LoginScreen extends Component {
  ...
  checkIfUserNameExistInDatabase = (userName, status) => {
    console.log('checkIfUserNameExistInDatabase : username : ' + userName);
    firebase
      .database()
      .ref('UsersList/')
      ...
      .once('value', snapshot => {
        // console.log(2
        //   'checkIfUserNameExistInDatabase : ' + JSON.stringify(snapshot),
        // );
        if (snapshot.exists()) {
          snapshot.forEach(child => {
            this.setUserId(child.val().userId);
          });
          // this.setUserId(userId);
          this.startChatListScreen();
        } else {
          console.log('checkIfUserNameExistInDatabase : not exist');
          this.addDataToFirebase(userName, status);
        }
      })
      .catch(error => {
        //error callback
  ...
  addDataToFirebase = (userName, status) => {
    // console.log(
    //   'addDataToFirebase : ' + this.checkIfUserNameExistInDatabase(userName),
    // );

    // if (this.checkIfUserNameExistInDatabase(userName) === false) {
    ...
    return (
      <View style={styles.container}>
        ...
        <TouchableHighlight
          ...
          onPress={() =>
            this.checkIfUserNameExistInDatabase(this.state.fullName, 'active')
          }>
