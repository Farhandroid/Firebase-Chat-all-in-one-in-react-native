...
export default class ShowAllConnectedUserScreen extends Component {
  ...
  findOutUserListChattedWithUser = userChattedHistory => {
    ...
    Object.keys(userChattedHistory).map((key, index) => {
      this.findUnseenMessageCount(userChattedHistory[key]);
    });
  };
    ...
    firebase
      ...
      .on('value', function(snapshot) {
        if (snapshot.exists()) {
          ...
          // console.log(
          //   'findUnseenMessageCount : ' +
          //     JSON.stringify(json.unsceneMessageCount),
          // );
    ...
    firebase
      ...
      .on('value', function(snapshot) {
        if (snapshot.exists()) {
          ...
          // let data = self.state.userChattedUser;
          // let temp={};

          // console.log(
          //   'findChatUIDusersList : ' + JSON.stringify(json.user_2_id),
          // );
          //self.findOutUserListChattedWithUser(snapshot.toJSON());
        } else {
          console.log('getUserListChattedWithUser : not exist');
    ...
    firebase
      ...
      .once('value', snapshot => {
        ...
        if (snapshot.exists()) {
          ...
          //   self.setUserId(child.val().userId);
          // });
          let data = {};
          ...
          temp.push(data);
          self.setState({userChattedUser: temp});
          console.log(
            'getMessagePartnerUserName temp : ' + JSON.stringify(temp),
          );
        } else {
